...
In Linux OS, we can perform the Word to PDF conversion using .NET Core application. To deploy .NET Core application with Word to PDF conversion capabilities in Linux OS, the following NuGet packages needs to be referred in your .NET Core application.
...
Additionally, we required SkiaSharp.Linux helper NuGet package which can be generated by following steps.
...
4. Make sure the nuget.exe file is present along with SkiaSharp.Linux folder (in the parent folder of SkiaSharp.Linux folder). If not download it from here.
...
Essential DocIO provides settings while performing Word to PDF conversion mentioned below, 

### Fast rendering

This setting, allows to convert PDF faster by using direct PDF rendering approach rather than EMF rendering approach.

The following code table shows how to convert the Word document to PDF using direct PDF rendering approach, 
...
We can customize the TrueType fonts embedding in two ways as following,

#### EmbedFonts

This setting, allows to embed the particular font information (glyphs) from the TrueType fonts used for the rendered characters in converted PDF document.

The following code table shows how to embed the TrueType fonts into the converted PDF document.
...
This setting, allows to embed the complete font information (glyphs) from the TrueType fonts used in converted PDF document.

The following code table shows how to embed the complete TrueType fonts into the converted PDF document.
...
This setting, allows to determine the quality of the charts and JPEG images in the converted PDF document.

The following code table shows how to customize the image quality of charts and JPEG images in the converted PDF document.
...
This setting, reduce the Main Memory usage in Word to PDF conversion by reusing the identical images.

The following code table shows how to reduce the Main Memory usage while converting Word to PDF by reusing identical images.
...
This setting, allows to set the PDF conformance level.

The following code table shows how to set the PdfConformanceLevel while converting Word to PDF.
...
This setting, allows to include the alternate chunks while converting Word to PDF conversion. As default, it includes alternate chunks.

The following code table shows how to exclude the alternate chunk parts in Word to PDF conversion.
