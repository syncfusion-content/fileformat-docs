...
The [PdfStringLayouter](https://help.syncfusion.com/cr/file-formats/Syncfusion.Pdf.Base~Syncfusion.Pdf.Graphics.PdfStringLayouter.html) class of Essential PDF helps to determine the layout result of the string without actually drawing the string. The result of ``PdfStringLayouter`` is maintained in the instance of [PdfStringLayoutResult](https://help.syncfusion.com/cr/file-formats/Syncfusion.Pdf.Base~Syncfusion.Pdf.Graphics.PdfStringLayoutResult.html).

The following code snippet explains the behavior of ``PdfStringLayouter`` and ``PdfStringLayoutResult``.
...
//Initialize PDF graphics for the page
PdfGraphics graphics = page.Graphics;

//Create a standard font
PdfFont font = new PdfStandardFont(PdfFontFamily.Helvetica, 30);

//Load text into a sting variable
string text = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla et libero vel lacus ultricies rutrum. Morbi nisl. Aliquam laoreet vulputate tortor‘ Suspendisse blandit. Morbi sodales, odio in pretium volutpat.";
            
//Initialize bounds to draw the text
RectangleF border = new RectangleF(0, 0, page.GetClientSize().Width, 150);

//Draw the rectangle with initialized bounds
graphics.DrawRectangle(PdfPens.Black, border);

//Initialize PdfStringLayouter
PdfStringLayouter layouter = new PdfStringLayouter();

//Initialize PdfStringLayoutResult to determine whether the string is clipped or not
...
//Resize the standard font if there is clipped text present with the given bounds value
if (result.Remainder != null && result.Remainder.Length != 0)
{
    //Method which resize the font size to fit the text inside the specified bounds
    PdfFont resizedFont = GetResizedFont(text, border, font);
    //Draw the text on PdfPage
...
//Resize the standard font
private static PdfFont GetResizedFont(string text, RectangleF bounds, PdfFont font)
{
    var fontSize = font.Size;
    while (fontSize > 0f && font.MeasureString(text, bounds.Width).Height > bounds.Height)
    {
        fontSize -= 0.1f;
        if (font is PdfCjkStandardFont)
            font = new PdfCjkStandardFont((PdfCjkStandardFont)font, fontSize, font.Style);
        else if (font is PdfStandardFont)
            font = new PdfStandardFont((PdfStandardFont)font, fontSize, font.Style);
        else if (font is PdfTrueTypeFont)
            font = new PdfTrueTypeFont((PdfTrueTypeFont)font, fontSize);
    }
...
'Load text into a sting variable
Dim text As String = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla et libero vel lacus ultricies rutrum. Morbi nisl. Aliquam laoreet vulputate tortor‘ Suspendisse blandit. Morbi sodales, odio in pretium volutpat."
...
'Resize the standard font if there is clipped text present with the given bounds value
If ((Not (result.Remainder) Is Nothing) AndAlso (result.Remainder.Length <> 0)) Then
    'Method which resize the font size to fit the text inside the specified bounds
...
'Resize the standard font
Private Function GetResizedFont(text As String, bounds As RectangleF, font As PdfFont) As PdfFont
    Dim fontSize = font.Size
    While ((fontSize > 0F) AndAlso (font.MeasureString(text, bounds.Width).Height > bounds.Height))
        fontSize = (fontSize - 0.1F)
        If (TypeOf font Is PdfCjkStandardFont) Then
            font = New PdfCjkStandardFont(CType(font, PdfCjkStandardFont), fontSize, font.Style)
        ElseIf (TypeOf font Is PdfStandardFont) Then
            font = New PdfStandardFont(CType(font, PdfStandardFont), fontSize, font.Style)
        ElseIf (TypeOf font Is PdfTrueTypeFont) Then
            font = New PdfTrueTypeFont(CType(font, PdfTrueTypeFont), fontSize)
        End If
    End While
